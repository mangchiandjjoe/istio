...
func runServerWithSelectedAlgorithm(t *testing.T, algorithm string) {
	...
	adapterConfig = strings.Replace(adapterConfig, "__REDIS_SERVER_ADDRESS__",
		mockRedis.redisServer.Addr().String(), -1)
		...
		for name, allocated := range c.expected {
			if quota, ok := res.Quotas[name]; ok {
				if quota.GrantedAmount != allocated {
					t.Errorf("%v: Expected GrantedAmount: %v, Got: %v", id, allocated, quota.GrantedAmount)
				}
			} else {
