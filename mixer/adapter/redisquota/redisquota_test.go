	...
	for id, c := range cases {
		...
		for _, req := range c.request {
			...
			if qr.Amount != req.allocated {
				t.Errorf("%v: Expecting %d, got %d at %v", id, req.allocated, qr.Amount, req.timestamp)
			}
		}
	}
}
...
func TestHandleQuotaErrorMsg(t *testing.T) {
	cases := map[string]struct {
		...
		"Invalid response from the script": {
			...
			errMsg: []string{
				"failed to run quota script: Error",
				"invalid response from the redis server: [Invalid number], err: strconv.Atoi: parsing \"Invalid number\": invalid syntax",
			},
		},
	}
